...
export function NewItemSheet({
  ...
  languages: Language[]
  initial?: { content: string; source: string; language: string }
  onCancel: () => void
  ...
  const [language, setLanguage] = useState(initial?.language ?? languages[0]?.name ?? '')
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-10 pt-4">
        ...
        {isEdit && (
          <p className="mt-2.5 text-xs leading-relaxed text-ink-faint">
            提醒：如果修改了这里的原文内容，之前在这篇笔记里加的批注会被清空（批注是按原文位置定位的，原文变了位置就对不上了）。只改来源或语言分类不影响批注。
          </p>
        )}
      </div>
    </div>
